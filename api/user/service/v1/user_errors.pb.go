...
func IsMissingParentId(err error) bool {
	if err == nil {
		return false
	}
	e := errors.FromError(err)
	return e.Reason == ErrorReason_MISSING_PARENT_ID.String() && e.Code == 400
}

func ErrorMissingParentId(format string, args ...interface{}) *errors.Error {
	return errors.New(400, ErrorReason_MISSING_PARENT_ID.String(), fmt.Sprintf(format, args...))
}

func IsUserNotFount(err error) bool {
	if err == nil {
		return false
	}
	e := errors.FromError(err)
	return e.Reason == ErrorReason_USER_NOT_FOUNT.String() && e.Code == 404
}
